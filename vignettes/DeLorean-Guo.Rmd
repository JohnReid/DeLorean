...
```{r compareProfiles}
# load_all('..')
gp <- plot(dl.te, type="best.predictions", profile.color='orange',
           add.data=F, genes=dl.te$gene.map$gene)
gp <- plot.add.profiles(gp, dl=dl.pe , color='green',
                        genes=dl.te$gene.map$gene)
gp <- plot.add.profiles(gp, dl=dl.epi, color='blue',
                        genes=dl.te$gene.map$gene)
