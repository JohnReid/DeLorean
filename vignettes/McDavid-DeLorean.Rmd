...
set.seed(1)
max.cells <- min(getOption("McDavid.max.cells", 57))
...
```{r filterGenes}
max.genes <- min(getOption("McDavid.max.genes", 56))
genes.for.stan <- sample((dl$gene.meta %>% filter(pvalue > 10))$gene,
                         max.genes)
dl <- filter.genes(dl, genes=genes.for.stan)
...
print(gp)
dev.off()
png('McDavid-rmse-vs-rmse-adj.png', width=fig.width*1.75, height=fig.height*1.5,
    units="in", res=200)
print(gp)
dev.off()
...
gp <- (
    ...
    + geom_density()
    + geom_rug()
    + scale_x_continuous(name="RMSE (cell size adjusted)"))
pdf('McDavid-rmse-adj.pdf', width=fig.width, height=fig.height)
print(gp)
dev.off()
gp <- (
    ggplot(dl$peak.max.dist, aes(sample=rmse.adj))
    + stat_qq())
pdf('McDavid-rmse-adj-qq.pdf', width=fig.width, height=fig.height)
print(gp)
dev.off()
...
print(gp)
dev.off()
png('McDavid-peak-vs-max.png', width=fig.width*1.75, height=fig.height*1.5,
    units="in", res=200)
print(gp)
dev.off()
...
peak.capture.dist
# t-test
with(dl$peak.max.dist, t.test(x=peak.capture.dist,
                              y=rmse.adj,
                              alternative="greater"))
...
# peak capture RMSE.
with(dl$peak.max.dist, pnorm(peak.capture.dist,
                             mean=mean(rmse.adj),
                             sd=sd(rmse.adj),
                             lower.tail=F))
gp <- (
...
print(gp)
dev.off()
png('McDavid-capture-peaks.png', width=fig.width*3.5, height=fig.height*3,
    units="in", res=100)
print(gp)
dev.off()
...
print(gp)
dev.off()
gp <- plot(dl, type="profiles") + scale_x_continuous(name='Pseudotime',
                                                     limits=c(0, 3))
pdf('McDavid-profiles.pdf', width=fig.width*2, height=fig.height*2)
print(gp)
dev.off()
png('McDavid-profiles.png', width=fig.width*3.5, height=fig.height*3,
    units="in", res=100)
print(gp)
dev.off()
