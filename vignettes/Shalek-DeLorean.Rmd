...
```{r testCellSize}
library(broom)
gene.levels <- levels(dl$gene.meta$gene)
cell.levels <- levels(dl$cell.meta$cell)
shalek.l <- melt.expr(dl) %>% left_join(dl$cell.meta)
qplot(sample_n(shalek.l, 10000)$x)
(
    shalek.l
    %>% group_by(capture)
    %>% do(tidy(kruskal.test(x ~ cell, data=.)))
)
gene.means <- (
    shalek.l
    %>% group_by(gene)
    %>% summarise(mu=mean(x), prop.expr=mean(x>0))
    %>% filter(prop.expr > .5)
)
qplot(gene.means$mu)
...
cell.sizes.ah <- (
    shalek.l
    %>% filter(gene %in% gene.means$gene)
    %>% group_by(capture)
    %>% do(anders.huber.cell.sizes(.))
)
cell.sizes.ah
ggplot(cell.sizes.ah, aes(x=capture, y=size)) + geom_boxplot()

shalek.adj.l <- (
    shalek.l
    %>% filter(gene %in% gene.means$gene)
    %>% left_join(cell.sizes.ah)
    %>% mutate(x=x-size)
)
cell.sizes.ah.test <- (
    shalek.adj.l
...
ggplot(cell.sizes.ah.test, aes(x=capture, y=size)) + geom_boxplot()
dl$expr <- shalek.l %>% acast(gene ~ cell, value.var="x")
...
set.seed(1)
num.cells <- getOption("Shalek.max.cells", 12)
...
```{r}
# Calculate the roughness of the held out genes under the ordering induced
# by the pseudotime
roughness.tau <- apply(held.out.expr[,order(tau.for.sample(dl))],
                       1,
                       calc.roughness)
roughness.tau <- data.frame(
    roughness.tau=roughness.tau,
    gene=factor(names(roughness.tau), levels=dl$gene.meta$gene))
roughness.tau.mean <- mean(roughness.tau$roughness.tau)
# Calculate the roughness over some random permutations
roughnesses <- sapply(1:1000, function(i) permuted.roughness(dl, held.out.expr))
dim(roughnesses)
# Plot the roughnesses
gp <- (
    qplot(colMeans(roughnesses))
    + geom_vline(x=roughness.tau.mean, linetype='dashed')
)
print(gp)
ggsave('Shalek-roughnesses-random.png', gp)
roughnesses.l <- (
    melt(roughnesses,
         varnames=c("gene", "permutation"),
         value.name="roughness")
    %>% mutate(gene=update.levels(gene, dl$gene.meta[['gene']])))
names(roughnesses.l)
roughnesses.gene <- (
    roughnesses.l
    %>% group_by(gene)
    %>% summarise(mu=mean(roughness), sigma=sd(roughness))
    %>% left_join(roughness.tau)
    %>% left_join(gene.variances)
    %>% mutate(Z=(mu-roughness.tau)/sigma)
    %>% arrange(Z))
gp <- ggplot(roughnesses.gene,
             aes(x=Psi/Omega, y=roughnesses.gene$Z)) + geom_point()
print(gp)
ggsave('Shalek-roughnesses-Z.png', gp)
with(roughnesses.gene, cor(sigma, Z))

```


We fit GPs to the data with two covariance functions. We use the
same covariance structure as the model.
$$
...
$$
```{r fitHeldOut}
marg.lls <- fit.held.out(dl, held.out.expr)
                         #sample.iter=dl$sample.optimised)
...
Look at expected sample variance given different parameters.
```{r expSampleVar}
lambda <- 3
K <- cov.calc.gene(dl,
                   include.test=F,
                   tau="capture",
                   psi=lambda * exp(dl$stan.data$mu_psi),
                   omega=lambda * exp(dl$stan.data$mu_omega))
expected.sample.var <- function(K) mean(diag(K)) - mean(K)
expected.sample.var(K)
exp(dl$stan.data$mu_psi) + exp(dl$stan.data$mu_omega)
