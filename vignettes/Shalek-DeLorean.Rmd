...
dl <- estimate.hyper(
    dl,
    sigma.tau=getOption("Shalek.sigma.tau", 2),
    delta=getOption("Shalek.delta", .5),
    length.scale=getOption("Shalek.length.scale", 5),
    model.name=getOption("Shalek.model", 'simple-model'))
# dl$gene.var %>% arrange(psi.hat)
dl$gene.var %>% arrange(omega.hat)
# stopifnot(FALSE)
...
marg.lls <- fit.held.out(dl, held.out.genes$gene, shalek.A.expr)
