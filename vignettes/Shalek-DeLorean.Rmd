...
max.genes <- min(getOption("Shalek.max.genes", 37))
...
held.out.genes <- with(dl, gene.var
                           ...
                           # %>% filter(! gene %in% gene.map$gene, cluster == "Id")
                           %>% arrange(-psi.bar))
best.sample <- with(dl,
...
marg.lls <- sapply(head(held.out.genes, 200)$gene,
...
held.out.gp <- (
    ...
    + geom_point()
    + scale_colour_gradient2(midpoint=mean(held.out.genes$psi.bar))
