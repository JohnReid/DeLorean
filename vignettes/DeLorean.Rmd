...
We will load this data and analyse a subset of it corresponding to
the epiblast lineage. First we must build a `de.lorean` object with the correct
data.
...
Estimate hyperparameters for the model from the whole data set.
```{r}
...
Now we have the data we can prepare it for fitting and plot the raw data of some
of the genes as violin plots.
```{r prepare}
