...
Compile and fit the model using Stan's ADVI variational Bayes (VB) algorithm. We could
also use one of Stan's samplers with `method='sample'`.
```{r fit, cache=T}
dl <- compile.model(dl);
dl <- find.best.tau(dl)
...
so that users of the samplers may know of it.
```{r}
plot(dl, type='Rhat')
```
