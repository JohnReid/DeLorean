...
tau.S.lower <- tau.S %>% tail(2000) %>% mutate(tau=tau-dl$opts$period)
tau.S.upper <- tau.S %>% head(2000) %>% mutate(tau=tau+dl$opts$period)
...
prediction.times <- (
    dl$best.mean
    %>% filter(1 == g, 0 <= tau, tau <= dl$opts$period)
    %>% select(t, tau))
...
The correlation between the capture time of the peak to the peak time in
[CycleBase](http://www.cyclebase.org/About "CycleBase") is
`r peak.capture.cor`.
