```{r build, echo=FALSE, eval=FALSE}
library(devtools)
load_all('..')
library(rmarkdown)
render('DeLorean-McDavid.Rmd')

```

---
title: DeLorean analysis of McDavid et al. cell cycle data
author: John Reid
bibliography: DeLorean.bib
output: html_document
---

<!--
%\VignetteEngine{knitr::rmarkdown}
%\VignetteIndexEntry{DeLorean analysis of McDavid et al. cell cycle data}
-->


```{r config, echo=FALSE, message=FALSE}
library(knitr)
library(knitcitations)
library(rmarkdown)
#
# knitr options
#
opts_chunk$set(
    fig.path = 'figures/McDavid-',
    stop_on_error = TRUE,
    fig.width = 12.5,
    fig.height = 8)
#
# Citations
#
cleanbib()
cite_options(
    # hyperlink = 'to.doc',
    hyperlink = TRUE,
    # style = 'html',
    # citation_format = 'text',
    citation_format = "pandoc",
    cite.style = "numeric",
    check.entries = TRUE)
    # hyperlink = TRUE)
bib <- read.bibtex("DeLorean.bib")
if (file.exists("config.R")) {
    source("config.R")
}

```

```{r init, echo=FALSE, message=FALSE, warning=FALSE}
# suppressMessages(loadfonts())
library(DeLorean)
#
# Stylesheet
#
options(markdown.HTML.stylesheet = system.file("inst/Rmd/foghorn.css",
                                               package="DeLorean"))
font.family <- "Verdana"
font.theme <- theme_update(text=element_text(family=font.family))
theme_set(font.theme)

```


`r citet(bib[["mcdavid_modeling_2014"]])` assayed actively cycling single
cells in order to examine the confounding effect of the cell cycle on
single cell expression analyses. They measured the expression of 333 genes
in 930 cells across three cell cycle phases and three cell lines.


# Data

McDavid et al.'s data is available in the `DeLorean` R package. Not all 333
genes are represented as several are rarely expressed.
```{r loadLib}
library(DeLorean)
data(McDavidDeLorean)

```

Shift positively expressed genes closer to unexpressed genes to improve fit.
```{r}
dl <- de.lorean(
    pmax(mcdavid.expr - .69, 0),
    mcdavid.gene.meta,
    mcdavid.cell.meta)

```

```{r child=report.file("data")}
```



# PCA analysis

```{r}
genes.high.rank <- (
    dl$gene.meta
    %>% filter(!is.na(cbRank), cbRank < 201)
    %>% arrange(cbRank))
levels(dl$cell.meta$cellline)
cells.PC3 <- (
    dl$cell.meta
    %>% filter("PC3" == cellline))
pca <- prcomp(t(mcdavid.expr[as.character(genes.high.rank$gene),
                             as.character(cells.PC3$cell)]),
              .scale=TRUE)
# Sqrt of eigenvalues
qplot(pca$sdev)
# Percentage of variance explained
qplot(pca$sdev**2 / sum(pca$sdev**2) * 100)
# Get the PC scores
pc.scores <- as.data.frame(pca$x)
pc.scores$cell <- factor(rownames(pca$x), levels=levels(dl$cell.meta$cell))
pc.scores <- pc.scores %>% left_join(dl$cell.meta)
# Plot PC1 against PC2
ggplot(pc.scores, aes(x=PC1, y=PC2, color=capture)) + geom_point()
# Plot PC1 against PC3
ggplot(pc.scores, aes(x=PC1, y=PC3, color=capture)) + geom_point()

```

# Estimate hyperparameters

Examine data for empirical Bayes estimation of hyperparameters.
```{r empiricalBayes}
dl <- estimate.hyper(
    dl,
    sigma.tau=.5,
    delta=.5)

```

```{r child=report.file("hyper-parameters")}
```


# Choose genes and cells

Choose a few cells from the PC3 cell line.
```{r filterCells}
set.seed(1)
max.cells <- min(getOption("DL.max.cells", 57))
cells.PC3 <- sample(filter(mcdavid.cell.meta, "PC3" == cellline)$cell,
                    max.cells)
cell.filter <- function(cells) cells %in% cells.PC3
dl <- filter.cells(dl, cell.filter)

```


Choose genes with low $p$-values from the McDavid et al.
differential expression test.
```{r filterGenes}
max.genes <- min(getOption("DL.max.genes", 56))
genes.for.stan <- sample((dl$gene.meta %>% filter(pvalue > 10))$gene,
                         max.genes)
gene.filter <- function(genes) genes %in% genes.for.stan
dl <- filter.genes(dl, gene.filter)

```


# Compile and fit model

Define and compile the model, find the best initialisation, and fit the model.
```{r model}
dl <- format.for.stan(dl, period=3, hold.out=3)
dl <- compile.model.simple(dl)
dl <- find.best.tau(dl)
system.time(dl <- fit.model(dl, num.cores=getOption("DL.num.cores")))

```


# Examine convergence.

```{r examConv}
dl <- examine.convergence(dl)

```

```{r child=report.file("convergence")}
```



# Analyse posterior

Examine posterior.
```{r posterior}
dl <- process.posterior(dl)
dl <- analyse.noise.levels(dl)

```

```{r child=report.file("posterior")}
```


# Profiles

Calculate expression profiles.
```{r makePredictions}
dl <- make.predictions(dl)
```

```{r child=report.file("profiles")}
```


# Cell size vs. pseudotime

```{r}
periodise <- function(tau, period=3) {
    wave = floor(tau/period)
    tau - wave * period
}
tau.S <- (
    dl$samples$tau
    %>% left_join(dl$samples$S)
    %>% mutate(tau=periodise(tau, dl$opts$period))
    %>% arrange(tau))
# Copy data to beginning and end to ensure periodic loess estimate
tau.S.lower <- tau.S %>% tail(2000) %>% mutate(tau=tau-dl$opts$period)
tau.S.upper <- tau.S %>% head(2000) %>% mutate(tau=tau+dl$opts$period)
tau.S <- rbind(tau.S.lower, tau.S, tau.S.upper)
# Fit loess
lo <- loess(S ~ tau, tau.S)
tau.S$loess <- predict(lo, tau.S)
# Make predictions at times used in predictedmean
prediction.times <- (
    dl$best.mean
    %>% filter(1 == g, 0 <= tau, tau <= dl$opts$period)
    %>% select(t, tau))
prediction.times$cell.size.adj <- predict(lo, prediction.times)
prediction.times
# Plot data + fit
png('cell-size-vs-pseudotime.png', width=1200, height=900)
(ggplot(tau.S, aes(x=tau, y=S, color=capture))
    + geom_point()
    + geom_line(aes(y=loess), color='black')
)
dev.off()

```

# Examine peak times

```{r peakTimes}

# Plot cbPeaktime against best sample max predicted mean time
# print(plot.best.predictions(dl, genes="CDCA8"))
dl <- within(dl, {
    peak.vs.max <- (best.mean %>% left_join(prediction.times %>% select(-tau))
        %>% group_by(g)
        %>% summarise(predicted.peak=tau[which.max(predictedmean)],
                      predicted.peak.adj=tau[which.max(predictedmean+cell.size.adj)])
        %>% left_join(gene.map))
    peak.max.cor <- with(
        peak.vs.max %>% filter(cbPeaktime > 10),
        cor(cbPeaktime,
            predicted.peak,
            use="complete.obs"))
    peak.max.cor.adj <- with(
        peak.vs.max %>% filter(cbPeaktime > 10),
        cor(cbPeaktime,
            predicted.peak.adj,
            use="complete.obs"))
})
dl$peak.max.cor
dl$peak.max.cor.adj

gp <- (
    ggplot(dl$peak.vs.max, aes(x=cbPeaktime, y=predicted.peak))
    + geom_point()
)
print(gp)
png('peak-vs-max.png', width=1200, height=900)
print(gp)
dev.off()

gp <- (
    ggplot(dl$peak.vs.max, aes(x=cbPeaktime, y=predicted.peak.adj))
    + geom_point()
)
print(gp)
png('peak-adj-vs-max.png', width=1200, height=900)
print(gp)
dev.off()

```

Calculate correlation based on estimate of peak time from capture time.
```{r}
expr.l <- melt(unname(dl$expr), varnames=c("g", "c"), value.name="x")
names(expr.l)
peaks <- (
    expr.l
    %>% group_by(g)
    %>% summarise(c=c[which.max(x)])
    %>% left_join(dl$cell.map)
    %>% left_join(dl$gene.map))
peak.capture.cor <- with(
    peaks,
    cor(cbPeaktime, as.integer(capture), use="complete.obs"))
peak.capture.cor
gp <- (
    ggplot(peaks %>% filter(! is.na(cbPeaktime)), aes(x=capture, y=cbPeaktime))
    + geom_boxplot()
    + coord_flip())
print(gp)
png('capture-peaks.png', width=1200, height=900)
print(gp)
dev.off()

```

The correlation between the capture time of the peak to the peak time in
[CycleBase](http://www.cyclebase.org/About "CycleBase") is
`r peak.capture.cor`.

Plot the profiles adjusted for the cell size.
```{r}
gp <- with(dl, {
    modulo.period <- function(t) t - floor(t / opts$period) * opts$period
    .data <- (best.mean
        %>% left_join(prediction.times, by="t")
        %>% left_join(gene.map)
        %>% filter(gene %in% genes.high.psi))
    (ggplot(.data, aes(x=modulo.period(tau)), environment=environment())
        + geom_line(alpha=.3,
                    data=.data,
                    aes(y=predictedmean+phi+cell.size.adj))
        + geom_ribbon(aes(ymin=predictedmean+phi+cell.size.adj-2*sqrt(predictedvar),
                          ymax=predictedmean+phi+cell.size.adj+2*sqrt(predictedvar)),
                      alpha=.1))
})
print(gp)
png('profiles-adj-cell-size.png', width=1200, height=900)
print(gp)
dev.off()


```


```{r echo=FALSE}
# Save DeLorean object without fit component
saveRDS({dl2 <- dl; dl2$fit <- NULL; dl2}, "McDavid.rds")
```


R version and packages used:
```{r Rversion}
sessionInfo()
```
