...
partition <- partition.de.lorean(dl, pieces=3)
```


# Investigate robustness
```{r robustness}
robustness.test <- test.robustness.de.lorean(dl)
print(robustness.test$gp)
...
#
# Investigate difference in noise levels between genes
#
(ggplot(dl$samples.l$omega %>% left_join(dl$gene.map),
        aes(x=factor(cbRank), y=omega))
    + geom_boxplot()
)
...
#
# Try to match maximal expression to peak time
#
find.maximum <- function(g) {
    with(dl, {
        maximum <- which.max(stan.data$expr[g,])
        tau <- (dl$samples.l$tau
                %>% filter(iter == best.sample,
                           c == maximum))$tau
        return(tau)
    })
}
maxima.unadj <- (dl$gene.map %>% mutate(max.loc=Vectorize(find.maximum)(g)))
(ggplot(maxima.unadj, aes(x=cbPeaktime, y=max.loc))
    + geom_point()
)
