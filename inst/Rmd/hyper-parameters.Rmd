...
$$ \begin{eqnarray*}
\sigma_\tau = `r stan.data$sigma_tau`
\delta = `r opts$delta`
l = `r stan.data$l`
\end{eqnarray*} $$
...
$$
\hat{S}_c = \textrm{Median}_g (x_{g,c} - \mu_g)
$$
...
```
```{r cellSizeFactorsByCapture}
(ggplot(cell.expr %>% left_join(cell.meta),
...
\end{align}
$$
The standard deviations have this distribution:
```{r geneWithinVar}
(ggplot(gene.time.expr, aes(x=sqrt(x.var))) + geom_density() + geom_rug())
