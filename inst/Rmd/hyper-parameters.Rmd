...
This document describes how the DeLorean package estimates the hyper parameters
for its model. We do not describe the model in this document. In this data set
we have expression data for $G=`r nrow(dl$gene.meta)`$ genes and
$C=`r nrow(dl$cell.meta)`$ cells, $x_{g,c}$, where $1\le g \le G$ and
$1 \le c \le C$.
...
distribution. The estimated cell size factsrs have this density
...
```
we again show the hyperparameters as the dotted blue line.
