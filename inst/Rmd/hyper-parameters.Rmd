...
expect some of the variation of expression within a time point to be associated
with variation in the temporal dimension. The rest of the variation within
a time point will be modelled as noise.  Conversely we expect most of the
variation between time points to be due to temporal variation although some
will be due to noise.
...
show the distribution of the $\log \hat{\omega}_g$ estimated by empirical Bayes).
```{r omegaPrior}
ggplot(
    gene.var,
    aes(x = log(omega.hat))) +
...
The vertical line represents the square root of the mean variance of the gene expressions.
```{r cellSizeRelExpr}
