...
```{r noiseScatter}
(ggplot(noise.levels,
        aes(x=psi, y=omega, color=gene %in% de.lorean$genes.high.psi)) + geom_point())
```

Measurement noise:
```{r measurementNoise}
(ggplot(noise.levels,
        aes(x=gene, y=log(omega), fill=gene %in% de.lorean$genes.high.psi))
    + geom_boxplot())
```
...
```{r temporalVariation}
(ggplot(noise.levels,
        aes(x=gene, y=log(psi), fill=gene %in% de.lorean$genes.high.psi))
    + geom_boxplot())
```
...
(ggplot(gene.noise.levels,
        aes(x=psi, y=omega, color=gene %in% de.lorean$genes.high.psi))
    + geom_point())
```
