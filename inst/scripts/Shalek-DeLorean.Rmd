...
genes.for.stan <- (
    ...
    %>% arrange(- Psi / Omega)
    %>% head(getOption("Shalek.max.genes", 74)))
...
sampled.cells <- sample(colnames(dl$expr), getOption("Shalek.max.cells", 307))
...
method <- getOption("Shalek.method", "vb")
system.time(dl <- fit.model(dl, method=method, num.inits=num.inits))
if ("vb" == method) {
...
```{r examConv, eval=("sample" == method)}
dl <- examine.convergence(dl)
```
```{r child=report.file("convergence"), eval=("sample" == method)}
```
