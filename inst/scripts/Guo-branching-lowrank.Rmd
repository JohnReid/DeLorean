...
# Calculate colours ggplot2 would use for continuous colour gradient
# cc <- scales::seq_gradient_pal("blue", "red", "Lab")(seq(0,1,length.out = length(levels(pca.df$capture))))
gp.pca <-
    ggplot(pca.df,
    ...
    theme_few()
ggsave('Plots/Guo-PCA.pdf', gp.pca, width = 12, height = 8, units = 'in')
```

Plot the cells' locations on first two PCA components with the direction that best describes capture time progression.
```{r plotPCA}
print(gp.pca)
```
...
gp.pca.z <- ggplot(pca.df,
    ...
    theme_few()
ggsave('Plots/Guo-PCA-z.pdf', gp.pca.z, width = 12, height = 8, units = 'in')
```

Plot the distance from each cell's location in 2D PCA space to the axis describing pseudotime.
We will use this as our initial estimate of our latent dimension $z$ in our model.
```{r plotPCAZ}
print(gp.pca.z)
```

Plot a histogram of the $z$ estimates.
```{r plotZhistogram}
qplot(pca.df$z.hat) + theme_few()
```
...
dl$stan.data$lengthscales = c(dl$stan.data$l, 6)  # Length scale for pseudotime and z
num.sd.border <- 5
num.inducing.tau <- 30
num.inducing.latent <- 5
dl$stan.data$M <- num.inducing.tau * num.inducing.latent
dl$stan.data$u <- with(dl, {
    taupseudo <- seq(
...
Compile
```{r compile}
model.path <- system.file('inst/Stan/branching-lowrank.stan', package = 'DeLorean')
...
create.init <- function() with(dl$stan.data, list(
        tauoffset = rnorm(C, sd = sigma_tau),
        z = dl$cell.map$z.hat,
...
```{r tauVsZ}
init.df <- data.frame(c = 1:dl$stan.data$C, tau.init = init$tauoffset + dl$stan.data$time, z.init = init$z)
...
gp.tau.z <-
    ...
    scale_colour_few() +
    theme_few()
ggsave('Plots/Guo-tau-z.pdf', gp.tau.z, width = 12, height = 8, units = 'in')
```
...
gp.var <-
    ...
    geom_label(alpha = .8) +
    theme_few()
ggsave('Plots/Guo-var.pdf', gp.var, width = 12, height = 8, units = 'in')
```
...
qplot(as.vector(dists.gene)) + theme_few()
dists.weighted <- exp(-dists.gene**2 / .7**2) * dists.latent / dists.gene
diag(dists.weighted) <- 0
ggplot(mutate(tau.z, weighted.dist = rowSums(dists.weighted)),
...
gp.gex <-
    ...
    facet_wrap(~ gene) +
    theme_few()
ggsave('Plots/Guo-gene-expr.pdf', gp.gex, width = 12, height = 8, units = 'in')
```
