...
gp.pca <-
    ...
    theme_few()
ggsave('Guo-PCA.pdf', gp.pca, width = 12, height = 8, units = 'in')
```

Plot the cells' locations on first two PCA components with the axis that best describes capture time progression.
```{r plotPCA}
print(gp.pca)
```
...
gp.pca.z <- ggplot(pca.df,
    ...
    theme_few()
ggsave('Guo-PCA-z.pdf', gp.pca.z, width = 12, height = 8, units = 'in')
```
...
gp.tau.z <-
    ...
    scale_colour_few() +
    theme_few()
ggsave('Guo-tau-z.pdf', gp.tau.z, width = 12, height = 8, units = 'in')
```
...
gp.var <-
    ...
    geom_label(alpha = .8) +
    theme_few()
ggsave('Guo-var.pdf', gp.var, width = 12, height = 8, units = 'in')
```
...
gp.gex <-
    ...
    facet_wrap(~ gene) +
    theme_few()
ggsave('Guo-gene-expr.pdf', gp.gex, width = 12, height = 8, units = 'in')
```
