...
Shalek et al. investigated mouse bone-marrow-derived dendritic cells. Two
matrix files are available from GEO. We shall name them A and B. First we
parse A.
```{r parseA}
...
cell.re.A <- str_c(
    '^',
    '(?:(IFNB|Tube|On_Chip|Ifnar1|Stat1|Tnfr)_)?',
    '(?:(KO|Control|Stimulation)_)?',
    '(?:(Unstimulated|LPS|PAM|PIC)_)?',
    '(?:([[:digit:]]+)h_)?',
    '(?:(Replicate)_)?',
    '(?:(S[[:digit:]]+)_)?',
    '$')
cell.meta.A <- (
    ...
    %>% select(cell=V1, genetic=V2, control=V3, stimulant=V4,
               capture=V5, replicate=V6, cell.id=V7)
...
nrow(cell.meta.A)
# cell.meta.A %>% filter("LPS"==stimulant, "" == genetic)
# cell.meta.A %>% filter("PIC" == stimulant, 2==capture)
# cell.meta.A %>% filter("Stat1" == genetic)

gene.meta.A <- data.frame(gene=genes.A)
...
supp.table.2 <- (
    ...
    %>% mutate(cell.orig=cell,
               cell.1=cell
                      %>% str_replace("Technical_Replicate_1_", "")
                      %>% str_replace("Technical_Replicate_2_", "Replicate_")
...
cell.meta.A <- cell.meta.A %>% left_join(supp.table.2 %>% select(cell, total, aligned))
# supp.table.2 %>% filter(is.na(cell))

# cell.meta.A %>% filter("LPS" == stimulant, "4" == capture, "" == genetic)
# cell.meta.A %>% filter("On_Chip" == genetic)
# sapply(cell.meta.A %>% select(-1), levels)
...
```{r saveData}
shalek.A.expr <- as.matrix(shalek.A)
shalek.A.cell.meta <- shalek.cell.meta.A
shalek.A.gene.meta <- shalek.gene.meta.A
save(shalek.A.expr,
     shalek.A.gene.meta,
     shalek.A.cell.meta,
