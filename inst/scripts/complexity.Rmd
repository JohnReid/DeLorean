...
number.of.cells <- c(10, 18, 30, 55, 100, 180, 300)
number.of.cells <- c(10, 18, 30)
cell.timings <- as.data.frame(t(sapply(
    number.of.cells,
...
cell.timings <-
    ...
    mutate(
        ...
        seed=seed)
number.of.genes <- c(10, 20, 30, 40, 50, 60, 70)
number.of.genes <- c(10, 20, 30)
gene.timings <- as.data.frame(t(sapply(
    number.of.genes,
    function(num.genes) time.fit(filter.cells(filter.genes(dl, number=num.genes), number=30))
)))
gene.timings <-
    gene.timings %>%
    mutate(
...
```{r plotTimings}
ggplot(gene.timings, aes(x=num.genes, y=elapsed)) + geom_line()
ggplot(cell.timings, aes(x=num.cells, y=elapsed)) + geom_line()
```
