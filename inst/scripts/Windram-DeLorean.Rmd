...
plots$profiles <- plot(dl, type="profiles", genes=dl$genes.high.psi[1:16])
...
ggsave('Windram-profiles.png', plots$profiles + plos.theme,
       width=6, height=3, dpi=600, units="in")
# Save tau posterior
plots$tau.posterior <- (
...
plots$roughnesses <- plot(dl, type="roughnesses") +
    geom_vline(xintercept=monocle.roughness, linetype='dashed', color='red')
