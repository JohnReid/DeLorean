...
dl <- estimate.hyper(dl, sigma.tau=group.size/2, model.name='lowrank')
...
dl <- prepare.for.stan(dl, num.inducing=14)
...
dl <- compile.model(dl)
# dl <- find.best.tau(dl, num.cores=1)
dl <- find.best.tau(dl, num.tau.candidates=20000)
# system.time(dl <- fit.model(dl, method='vb'))
system.time(dl <- fit.model(dl, method='sample', iter=5000))
```
