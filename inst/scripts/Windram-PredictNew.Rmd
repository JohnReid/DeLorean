...
gp <- add.held.out.to.profiles(dl, profiles.plot(dl))
gp
```
...
```{r combineHeldOut}
ggplot(
    dl$held.out$combined %>% left_join(dl$cell.meta),
    aes(x = obstime.orig, y = tau, colour = capture, shape = fit)) +
  geom_point(size = 5)
```
