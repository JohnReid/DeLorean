...
inf.method <- getOption("Windram.method", "sample")
```
...
plots <- list()
plots$orderings <- plot(dl, type='orderings')
plots$orderings
...
if ("vb" == inf.method) {
  ...
  system.time(dl <- fit.model(dl, method = inf.method, chains = num.inits))
}
```
...
held.out.best.time <-
  held.out.samples %>%
  left_join(filter(dl$predictions, dl$best.sample == iter)) %>%
  group_by(cell, t) %>%
  summarise(
    ll = sum(dnorm(
      expr,
...
```{r plotFitWithHeldOut}
gp <-
  profiles.plot(dl) +
  geom_point(
    data = filter(held.out.estimates, gene %in% dl$genes.high.psi),
    shape = 4, size = 7,
...
samples.combined <-
  bind_rows(
    dl$samples.l$tau %>%
      filter(iter == dl$best.sample) %>%
      select(cell, tau, capture, unobsfuscated = obstime.orig) %>%
      mutate(fit = 'original'),
    held.out.best.time %>%
      select(cell, tau, capture, unobsfuscated = obstime.orig) %>%
      mutate(fit = 'held out')) %>%
...
ggplot(
    samples.combined,
    aes(x = unobsfuscated, y = tau, colour = capture, shape = fit)) +
  geom_point(size = 5)
```
